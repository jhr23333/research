...
- **/discover**（项目skill）：覆盖池外候选发现。串行跑五个维度脚本（基本面催化/估值/覆盖稀疏/产业链溢出/另类数据映射）扫描半导体设备指数（884229.TI）~24支，自动合并为单一摘要 md；Python 侧完成全部打分与交集计算，LLM 只读 `summary_YYYYMMDD.md`（~3-5K token）
- **/quick-memo**（项目skill）：临时资料库一次性memo。投入 `_临时\` 的 PDF/Word/PPT/Excel/md，按当次具体要求生成 memo 存回 `_临时\`，然后**自动删除全部原始资料**。PDF 默认 pymupdf 极速抽取（表格密集时切 marker-pdf），不进入正式知识库——长期归档请走 `/kb-add`
...
03_主题\          ← 跨公司主题研究 + explorations/ + 候选池/ + 行业级原始资料
04_另类数据\      ← 高频另类数据（scripts / data / 看板）
99_模板\          ← 各类研究文档的标准模板
inbox\            ← 长期归档资料暂存口（PDF/Word/PPT），由 /kb-add 处理
Clippings\        ← Obsidian Web Clipper 网页剪藏收件箱，由 /kb-add Clippings 处理
_临时\            ← 一次性资料暂存口（/quick-memo 处理后自动清空）
scripts\          ← 自动化工具（search.py / discover/ / hooks/ / model.py）
```
