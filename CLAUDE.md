...
2. 用列表点反馈和总结
3. 先规划再执行
4. **工作流变更必须同步文档**：新增目录、修改skill、改变设计原则时，同步更新 CLAUDE.md 和相关 SKILL.md，不等用户提醒
...
- **/alt-data**（项目skill）：拉取 05_另类数据 登记的 iFind EDB 指标 + search_notice 资本开支公告，检查信号阈值；**仅负向/异常信号**写入假设.md，正向趋势只记录在看板

## 目录结构

```
01_公司\          ← 公司研究（memo / 基本面 / 假设 / 纪要 / 研报 / 年报）
02_产业链节点\    ← 技术、供应商、工艺节点（双向链接）
03_主题\          ← 跨公司主题研究 + explorations/
04_原始资料\      ← 归档的原始文件（公告财报 / 新闻剪报）
05_另类数据\      ← 高频另类数据（scripts / data / 看板）
```

**05_另类数据 设计原则**：每条数据序列必须关联到具体公司的具体假设（H#），孤立数据不入库。信号触发时自动追加到对应公司的 `假设.md` 待核实问题。
